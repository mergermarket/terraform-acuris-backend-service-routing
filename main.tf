...
locals {
  ...
  logical_service_name = "${length(regex("^live(_.+)?$", var.env)) > 0 && var.aws_account_alias == "" ? replace(var.component_name, "/-service$/", "") : "${var.env}-${replace(var.component_name, "/-service$/", "")}"}"
  # full_account_name    = "${var.env == "live" ? "${var.aws_account_alias}prod" : "${var.aws_account_alias}dev"}"
  full_account_name    = "${length(regex("^live(_.+)?$", var.env)) > 0 ? (var.aws_account_alias == "" ? "" : "${var.aws_account_alias}prod.") : "${var.aws_account_alias}dev."}"
